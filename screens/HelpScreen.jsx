...
export default function HelpScreen({ onNavigate, themeColors }) {
  ...
  // Componentes de texto para estandarizar párrafos y negritas
  ...
  // ✅ Nuevo componente para las negritas que resalta con el color principal del texto
  const B = ({ children }) => <Text style={{ fontWeight: 'bold', color: themeColors.text }}>{children}</Text>;

  // Función para abrir el correo
  const handleSendEmail = () => {
    Linking.openURL('mailto:hello.inventariaje@gmail.com?subject=Soporte%20Inventariaje%20App');
  };
    ...
    <View style={[GLOBAL_STYLES.container, { backgroundColor: themeColors.bg }]}>
      ...
      >
        ...
        <AccordionItem title="Ventas" icon={<FontAwesome6 name="money-bill-trend-up" size={16} color="black" />} themeColors={themeColors}>
          ...
          <Bullet><B>Bono Influencer:</B> Marca esta casilla cuando quieras registrar salida de un producto que adquiriste bajo Bono Influencer, de esta forma tu inventario te mostrará cuantas piezas a precio especial tienes disponibles.</Bullet>
        ...
        <AccordionItem title="Existencias" icon={<FontAwesome6 name="clipboard-check" size={16} color="black" />} themeColors={themeColors}>
          ...
          <Bullet><B>Notas:</B> Puedes agregar notas o recordatorios a cada producto para tener información adicional. (éstas no afectan ni modifican la cantidad de existencias).</Bullet>
        </AccordionItem>
       ...
       {/* ================= SECCIÓN 2 ================= */}
        ...
        <AccordionItem 
          ...
          <Bullet><B>Eventos de Escaner:</B> Monto cobrado por escaneos (sin contar venta de producto).</Bullet>
          <Bullet><B>Flujo de Efectivo:</B> La liquidez de tu negocio en ese período de tiempo. Considera cobros de escaner, ventas totales y descuenta los gastos de inventario (Ojo: Este es el único KPI que puede dar negativo. Si hoy no vendiste nada ($0) pero compraste producto por $5,000, tu flujo de efectivo será de -$5,000).</Bullet>
          <Bullet><B>Margen de Ganancia:</B> Es la rentabilidad, resultado del monto cobrado menos el costo real del producto (incluyendo descuentos).</Bullet>          
          <Bullet><B>Cortesías:</B> Costo del producto ofrecido de forma gratuita o usado como consumo personal.</Bullet>
        ...
        <AccordionItem title="Dudas y Comentarios" icon={<Ionicons name="chatbubble-ellipses-sharp" size={16} color="black" />} themeColors={themeColors}>
          ...
          {/* ✅ Botón de Correo Electrónico Enlazado */}
          <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  // ✅ Nuevos estilos para el botón del correo
  emailBtn: {
    marginTop: 15,
    marginBottom: 5,
    paddingVertical: 12,
    ...
    backgroundColor: 'rgba(36, 197, 197, 0.05)', // Fondo sutil para dar aspecto cliqueable
    alignItems: 'center',
